...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if insect.name == 'Queen' and insect.true_queen:
            return
        if insect.is_ant:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        p = self.place
        range = 0
        while not p.bees and p.entrance is not hive:
            if p.entrance:
                p = p.entrance
                range += 1
        if range < self.min_range or range > self.max_range:
            return None
        else:
...
class FireAnt(Ant):
    ...
    "*** YOUR CODE HERE ***"
    food_cost = 4
    implemented = True #tells the game that you've added a new type of Ant
    ...
    def reduce_armor(self, amount):
        ...
        if not place0.ant:
            for b in place0.bees[:]: #If you need to modify the list you are iterating over, you must iterate over a copy
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        "*** YOUR CODE HERE ***"
        print("there's", len(self.place.bees))
...
class HungryAnt(Ant):
    ...
    # def __init__(self, armor=1, turnsleft=0):
    #     Ant.__init__(self, armor)
    #     self.digesting = turnsleft
    ...
    def action(self, colony):
        ...
        if self.digesting != 0:
            self.digesting -= 1
            return
        somebee = random_or_none(self.place.bees)
        if somebee is not None:
            self.eat_bee(somebee)
...
class QueenPlace:
    ...
    def __init__(self, colony_queen, ant_queen):
        "*** YOUR CODE HERE ***"
        self.place0 = colony_queen
        self.place1 = ant_queen

    @property
    def bees(self):
        "*** YOUR CODE HERE ***"
        if self.place0 is self.place1:
            return self.place0.bees
        else:
            return self.place0.bees + self.place1.bees
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        self.true_queen = QueenAnt.queen_flag
        QueenAnt.queen_flag = False
        self.inspired_ants = [self]
    ...
    def action(self, colony):
        ...
        if not self.true_queen:
            ...
            return
        #relace colony.queen with QueenPlace
        colony.queen = QueenPlace(colony.queen, self.place)

        ScubaThrower.action(self, colony)

        def double_attack(place):
            #double the attack of ant on this place(including those with
            #BodyguardAnt)
            if place.ant is not None and \
               place.ant not in self.inspired_ants:
                place.ant.damage *= 2
                self.inspired_ants.append(place.ant)
                if place.ant.container and \
                   place.ant.ant is not None and \
                   place.ant.ant not in self.inspired_ants:
                    place.ant.ant.damage *= 2
                    self.inspired_ants.append(place.ant.ant)
        #Bodyguard on queen's face

        ent, ext = self.place, self.place.exit
        while ent is not colony.hive:
            double_attack(ent)
            ent = ent.entrance
...
def apply_effect(effect, bee, duration):
    ...
    "*** YOUR CODE HERE ***"
    normal_action = bee.action
    def effected_action(colony):
        nonlocal duration
        if duration == 0:
            return normal_action(colony)
        else:
            duration -= 1
            return effect(normal_action)(colony)
